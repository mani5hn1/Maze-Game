...
namespace Maze_Game
{
    ...
    {
        //This is Room 2 screen where the boarders are a bit thick and a bit challenging to play through.
        //Collection is a the count of coins that are collected and Collision is the number of times the cursor hits the walls.
        ...
        private void SubmitScoreButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string query = string.Format("Insert [MazeGame] ([Username], [Time], [CoinsCollected], [Room], [Collisions]) values ('{0}','{1}',N'{2}','{3}','{4}')", LoginScreen.Username, string.Format("{00}", sw.Elapsed.Seconds + " seconds"), collection, "Room 2", collision);
                    ...
                    cnn.Close();
                    string title = "Close Window";
                    MessageBox.Show("Your Score was submitted!", title, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                else
                {
